...
def as_worker(method):
    def wrapper(self, *args, use_worker=True, **kwargs):
        if use_worker and self._runner:
            ...
            return self._runner.run_worker(
                blocking, exclusive=False, thread_name="dbmgr"
            )
        return method(self, *args, use_worker=False, **kwargs)
...
class DbMgr:
    ...
    def find_one_and_update(
        self, col_name, filter, update, return_document=True, use_worker=True
    ):
        col = self.get_collection(col_name)
    ...
    def insert_uniq_by_timestamp(self, collection, jdoc):
        ...
        existing = self.find_one(
            collection, {"doc_type": doc_type, "timestamp": timestamp}
        )
        if not existing:
