...
class TopBar(Container):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.sender = sender
        self.tb_version = Label(
            Text.from_markup(f"[b {hi}]Db4E[/] v[{hi}]{app_version}[/]"),
            id="topbar_version",
        )
        self.tb_title = Label(
            Text.from_markup(f"[b {hi}]{title} - {sub_title}[/]"), id="topbar_title"
        )
    ...
    def watch_tb_state(self, old: TopBarState, new: TopBarState) -> None:
        self.tb_title.update(
            Text.from_markup(
                f"[b {hi}]{self.tb_state.title}[/] - [{hi}]{self.tb_state.sub_title}[/]"
            )
            # Text('[Title - SubTitle]')
        )
