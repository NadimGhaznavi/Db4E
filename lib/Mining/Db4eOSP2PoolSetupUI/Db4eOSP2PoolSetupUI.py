...
class Db4eOSP2PoolSetupUI:
    ...
    def on_submit(self, button):
        ...
        p2pool_prog = self.ini.config['p2pool']['process']
        p2pool_src = os.path.join(db4e_dir, third_party_dir, p2pool_dir, bin_dir, p2pool_prog)
        p2pool_dest = os.path.join(vendor_dir, p2pool_dir, bin_dir, p2pool_prog)
        if not os.path.exists(os.path.join(vendor_dir, p2pool_dir, bin_dir)):
            os.mkdir(os.path.join(vendor_dir, p2pool_dir, bin_dir))
        shutil.copy(p2pool_src, p2pool_dest)

        # Copy the start script from the db4e repo into the user's vendor directory
        start_script = self.ini.config['p2pool']['start_script']
        start_script_src = os.path.join(db4e_dir, third_party_dir, p2pool_dir, bin_dir, start_script)
        start_script_dest = os.path.join(vendor_dir, p2pool_dir, bin_dir, start_script)
