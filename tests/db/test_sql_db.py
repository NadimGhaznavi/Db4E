...
def test_uninitialized_sql_db_is_uninitialized(uninitialized_bootstrap_mgr):
    sql_db = SQLDb(db_type=DField.SERVER, bs_mgr=uninitialized_bootstrap_mgr)
    assert sql_db.is_initialized() is False


def test_initialized_sql_db_is_initialized(initialized_bootstrap_mgr):
    sql_db = SQLDb(db_type=DField.SERVER, bs_mgr=initialized_bootstrap_mgr)
    assert sql_db.is_initialized() is True


def test_creates_sync_meta(initialized_bootstrap_mgr):
    sql_db = SQLDb(db_type=DField.SERVER, bs_mgr=initialized_bootstrap_mgr)
...
def test_unitialized_sql_db_check_initialized(uninitialized_bootstrap_mgr):
    sql_db = SQLDb(db_type=DField.SERVER, bs_mgr=uninitialized_bootstrap_mgr)
    sql_db.check_initialized()
    assert sql_db.is_initialized() is False


def test_initialized_sql_db_check_initialized_server(initialized_bootstrap_mgr):
    sql_db = SQLDb(db_type=DField.SERVER, bs_mgr=initialized_bootstrap_mgr)
    sql_db.check_initialized()
    assert sql_db.is_initialized() is True


def test_initialized_sql_db_check_initialized_client(initialized_bootstrap_mgr):
    sql_db = SQLDb(db_type=DField.CLIENT, bs_mgr=initialized_bootstrap_mgr)
    sql_db.check_initialized()
    assert sql_db.is_initialized() is True


def test_close(initialized_bootstrap_mgr):
    sql_db = SQLDb(db_type=DField.SERVER, bs_mgr=initialized_bootstrap_mgr)
...
def test_execute_query(initialized_bootstrap_mgr):
    sql_db = SQLDb(db_type=DField.CLIENT, bs_mgr=initialized_bootstrap_mgr)
...
def test_executescript(initialized_bootstrap_mgr):
    sql_db = SQLDb(db_type=DField.CLIENT, bs_mgr=initialized_bootstrap_mgr)
...
def test_execute_merge(initialized_bootstrap_mgr):
    sql_db = SQLDb(db_type=DField.CLIENT, bs_mgr=initialized_bootstrap_mgr)
    sql_db.executescript(
        """
...
def test_find_one(initialized_bootstrap_mgr):
    sql_db = SQLDb(db_type=DField.CLIENT, bs_mgr=initialized_bootstrap_mgr)
    sql_db.executescript(
        """
        CREATE TABLE IF NOT EXISTS test (id INTEGER PRIMARY KEY, name TEXT, age INTEGER);
...
def test_find_many(initialized_bootstrap_mgr):
    sql_db = SQLDb(db_type=DField.CLIENT, bs_mgr=initialized_bootstrap_mgr)
    sql_db.executescript(
        """
        CREATE TABLE IF NOT EXISTS test (id INTEGER PRIMARY KEY, name TEXT, age INTEGER);
...
def test_get_last_sync(initialized_bootstrap_mgr):
    sql_db = SQLDb(db_type=DField.CLIENT, bs_mgr=initialized_bootstrap_mgr)
    sql_db.execute_query(
...
def test_get_max_updated_ts(initialized_bootstrap_mgr):
    sql_db = SQLDb(db_type=DField.CLIENT, bs_mgr=initialized_bootstrap_mgr)
    sql_db.executescript(
        """
...
def test_init_db(initialized_bootstrap_mgr):
    sql_db = SQLDb(db_type=DField.CLIENT, bs_mgr=initialized_bootstrap_mgr)
...
def test_sql_db_initialize(tmp_dir, uninitialized_bootstrap_mgr):
    sql_db = SQLDb(db_type=DField.CLIENT, bs_mgr=uninitialized_bootstrap_mgr)
    assert sql_db.is_initialized() is False
    assert sql_db._conn is None
    assert sql_db._cursor is None
    assert sql_db._initialized is False
...
def test_insert_one(initialized_bootstrap_mgr):
    sql_db = SQLDb(db_type=DField.CLIENT, bs_mgr=initialized_bootstrap_mgr)
    sql_db.executescript(
        """
        CREATE TABLE IF NOT EXISTS test (id INTEGER PRIMARY KEY, name TEXT);
...
def test_update_one(initialized_bootstrap_mgr):
    sql_db = SQLDb(db_type=DField.CLIENT, bs_mgr=initialized_bootstrap_mgr)
    sql_db.executescript(
        """
        CREATE TABLE IF NOT EXISTS test (id INTEGER PRIMARY KEY, name TEXT, age INTEGER, updated_ts INTEGER);
...
def test_update_last_sync(initialized_bootstrap_mgr):
    sql_db = SQLDb(db_type=DField.CLIENT, bs_mgr=initialized_bootstrap_mgr)
    sql_db.update_last_sync("monerod", 99)
    assert sql_db.get_last_sync("monerod") == 99
    sql_db.close()
