...
The *db4e* application supports generating custom scheduled reports using simple, standard YAML. A custom report definition is shown below:

```
# Pool Hashrate - 60 days
- report_type: hashrate
  sub_type: pool
...
Custom reports are generated by running `db4e.py -a run_reports`. The *db4e* application reads the report definitions, retrieves the data and publishes the results to the web. A [summary page](/pages/reports/index.html) of all reports is also generated.
