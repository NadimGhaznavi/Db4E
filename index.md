...
The *db4e* application supports defining custom reports using a simple YAML file. Custom reports are generated by running `db4e.py -a run_reports`. Generating reports can be scheduled with a cronjob. The *db4e* application reads the [reports definitions file](/conf/reports.yml), retrieves the necessary data and publishes the results to the web. A [summary page](/pages/reports/index.html) of all reports is also generated.
